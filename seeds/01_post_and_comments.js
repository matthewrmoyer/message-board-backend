exports.seed = function(knex, Promise) {
  ...
  return knex('comment').del()
    ...
    .then(function (postIds) {
      return Promise.all([
        knex('comment').insert({post_id: postIds[0], content: 'Firsties!'}),
        knex('comment').insert({post_id: postIds[0], content: 'I did it for the lulz'}),
        knex('comment').insert({post_id: postIds[2], content: 'This comment thread is so lonely'}),
      ])
    })
